import tkinter as tk
from tkinter import ttk, messagebox
import sys

class MinecraftFlatWorldGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("我的世界超平坦代码生成器 (Java版)")
        # 将窗口高度从650增加到700
        self.root.geometry("850x700")
        
        # 方块列表（包含ID和显示名称）
        self.blocks = [
            ("草方块", "minecraft:grass_block"),
            ("泥土", "minecraft:dirt"),
            ("石头", "minecraft:stone"),
            ("沙子", "minecraft:sand"),
            ("红沙", "minecraft:red_sand"),
            ("基岩", "minecraft:bedrock"),
            ("水", "minecraft:water"),
            ("岩浆", "minecraft:lava"),
            ("橡木木板", "minecraft:oak_planks"),
            ("玻璃", "minecraft:glass"),
            ("钻石矿石", "minecraft:diamond_ore"),
            ("铁矿石", "minecraft:iron_ore"),
            ("煤矿石", "minecraft:coal_ore"),
            ("下界岩", "minecraft:netherrack"),
            ("末地石", "minecraft:end_stone"),
            ("黏液块", "minecraft:slime_block"),
            ("雪块", "minecraft:snow_block"),
            ("冰", "minecraft:ice"),
            ("空气", "minecraft:air"), 
            ("仙人掌", "minecraft:cactus"), 
            ("金合欢木按钮", "minecraft:acacia_button"),
            ("金合欢木门", "minecraft:acacia_door"),
            ("金合欢木栅栏", "minecraft:acacia_fence"),
            ("金合欢木栅栏门", "minecraft:acacia_fence_gate"),
            ("悬挂式金合欢木告示牌", "minecraft:acacia_hanging_sign"),
            ("金合欢树叶", "minecraft:acacia_leaves"),
            ("金合欢原木", "minecraft:acacia_log"),
            ("金合欢木板", "minecraft:acacia_planks"),
            ("金合欢木压力板", "minecraft:acacia_pressure_plate"),
            ("金合欢树苗", "minecraft:acacia_sapling"),
            ("金合欢木告示牌", "minecraft:acacia_sign"),
            ("金合欢木台阶", "minecraft:acacia_slab"),
            ("金合欢木楼梯", "minecraft:acacia_stairs"),
            ("金合欢木活板门", "minecraft:acacia_trapdoor"),
            ("悬挂式金合欢木告示牌", "minecraft:acacia_wall_hanging_sign"),
            ("金合欢木告示牌", "minecraft:acacia_wall_sign"),
            ("金合欢木", "minecraft:acacia_wood"),
            ("激活铁轨", "minecraft:activator_rail"),
            ("绒球葱", "minecraft:allium"),
            ("紫水晶块", "minecraft:amethyst_block"),
            ("紫水晶簇", "minecraft:amethyst_cluster"),
            ("远古残骸", "minecraft:ancient_debris"),
            ("安山岩", "minecraft:andesite"),
            ("安山岩台阶", "minecraft:andesite_slab"),
            ("安山岩楼梯", "minecraft:andesite_stairs"),
            ("安山岩墙", "minecraft:andesite_wall"),
            ("铁砧", "minecraft:anvil"),
            ("结果的西瓜茎", "minecraft:attached_melon_stem"),
            ("结果的南瓜茎", "minecraft:attached_pumpkin_stem"),
            ("杜鹃花丛", "minecraft:azalea"),
            ("杜鹃树叶", "minecraft:azalea_leaves"),
            ("蓝花美耳草", "minecraft:azure_bluet"),
            ("竹子", "minecraft:bamboo"),
            ("竹块", "minecraft:bamboo_block"),
            ("竹按钮", "minecraft:bamboo_button"),
            ("竹门", "minecraft:bamboo_door"),
            ("竹栅栏", "minecraft:bamboo_fence"),
            ("竹栅栏门", "minecraft:bamboo_fence_gate"),
            ("悬挂式竹告示牌", "minecraft:bamboo_hanging_sign"),
            ("竹马赛克", "minecraft:bamboo_mosaic"),
            ("竹马赛克台阶", "minecraft:bamboo_mosaic_slab"),
            ("竹马赛克楼梯", "minecraft:bamboo_mosaic_stairs"),
            ("竹板", "minecraft:bamboo_planks"),
            ("竹压力板", "minecraft:bamboo_pressure_plate"),
            ("竹笋", "minecraft:bamboo_sapling"),
            ("竹告示牌", "minecraft:bamboo_sign"),
            ("竹台阶", "minecraft:bamboo_slab"),
            ("竹楼梯", "minecraft:bamboo_stairs"),
            ("竹活板门", "minecraft:bamboo_trapdoor"),
            ("悬挂式竹告示牌", "minecraft:bamboo_wall_hanging_sign"),
            ("木桶", "minecraft:barrel"),
            ("屏障", "minecraft:barrier"),
            ("玄武岩", "minecraft:basalt"),
            ("信标", "minecraft:beacon"),
            ("蜂巢", "minecraft:bee_nest"),
            ("蜂箱", "minecraft:beehive"),
            ("甜菜根", "minecraft:beetroots"),
            ("钟", "minecraft:bell"),
            ("大型垂滴叶", "minecraft:big_dripleaf"),
            ("大型垂滴叶茎", "minecraft:big_dripleaf_stem"),
            ("白桦木按钮", "minecraft:birch_button"),
            ("白桦木门", "minecraft:birch_door"),
            ("白桦木栅栏", "minecraft:birch_fence"),
            ("白桦木栅栏门", "minecraft:birch_fence_gate"),
            ("悬挂式白桦木告示牌", "minecraft:birch_hanging_sign"),
            ("白桦树叶", "minecraft:birch_leaves"),
            ("白桦原木", "minecraft:birch_log"),
            ("白桦木板", "minecraft:birch_planks"),
            ("白桦木压力板", "minecraft:birch_pressure_plate"),
            ("白桦树苗", "minecraft:birch_sapling"),
            ("白桦木告示牌", "minecraft:birch_sign"),
            ("白桦木台阶", "minecraft:birch_slab"),
            ("白桦木楼梯", "minecraft:birch_stairs"),
            ("白桦木活板门", "minecraft:birch_trapdoor"),
            ("悬挂式白桦木告示牌", "minecraft:birch_wall_hanging_sign"),
            ("白桦木告示牌", "minecraft:birch_wall_sign"),
            ("白桦木", "minecraft:birch_wood"),
            ("黑色旗帜", "minecraft:black_banner"),
            ("黑色床", "minecraft:black_bed"),
            ("黑色蜡烛", "minecraft:black_candle"),
            ("插上黑色蜡烛的蛋糕", "minecraft:black_candle_cake"),
            ("黑色地毯", "minecraft:black_carpet"),
            ("黑色混凝土", "minecraft:black_concrete"),
            ("黑色混凝土粉末", "minecraft:black_concrete_powder"),
            ("黑色带釉陶瓦", "minecraft:black_glazed_terracotta"),
            ("黑色潜影盒", "minecraft:black_shulker_box"),
            ("黑色染色玻璃", "minecraft:black_stained_glass"),
            ("黑色染色玻璃板", "minecraft:black_stained_glass_pane"),
            ("黑色陶瓦", "minecraft:black_terracotta"),
            ("黑色羊毛", "minecraft:black_wool"),
            ("黑石", "minecraft:blackstone"),
            ("黑石台阶", "minecraft:blackstone_slab"),
            ("黑石楼梯", "minecraft:blackstone_stairs"),
            ("黑石墙", "minecraft:blackstone_wall"),
            ("高炉", "minecraft:blast_furnace"),
            ("蓝色旗帜", "minecraft:blue_banner"),
            ("蓝色床", "minecraft:blue_bed"),
            ("蓝色蜡烛", "minecraft:blue_candle"),
            ("插上蓝色蜡烛的蛋糕", "minecraft:blue_candle_cake"),
            ("蓝色地毯", "minecraft:blue_carpet"),
            ("蓝色混凝土", "minecraft:blue_concrete"),
            ("蓝色混凝土粉末", "minecraft:blue_concrete_powder"),
            ("蓝色带釉陶瓦", "minecraft:blue_glazed_terracotta"),
            ("蓝冰", "minecraft:blue_ice"),
            ("兰花", "minecraft:blue_orchid"),
            ("蓝色潜影盒", "minecraft:blue_shulker_box"),
            ("蓝色染色玻璃", "minecraft:blue_stained_glass"),
            ("蓝色染色玻璃板", "minecraft:blue_stained_glass_pane"),
            ("蓝色陶瓦", "minecraft:blue_terracotta"),
            ("蓝色羊毛", "minecraft:blue_wool"),
            ("骨块", "minecraft:bone_block"),
            ("书架", "minecraft:bookshelf"),
            ("脑纹珊瑚", "minecraft:brain_coral"),
            ("脑纹珊瑚块", "minecraft:brain_coral_block"),
            ("脑纹珊瑚扇", "minecraft:brain_coral_fan"),
            ("脑纹珊瑚扇", "minecraft:brain_coral_wall_fan"),
            ("酿造台", "minecraft:brewing_stand"),
            ("红砖台阶", "minecraft:brick_slab"),
            ("红砖楼梯", "minecraft:brick_stairs"),
            ("红砖墙", "minecraft:brick_wall"),
            ("红砖块", "minecraft:bricks"),
            ("棕色旗帜", "minecraft:brown_banner"),
            ("棕色床", "minecraft:brown_bed"),
            ("棕色蜡烛", "minecraft:brown_candle"),
            ("插上棕色蜡烛的蛋糕", "minecraft:brown_candle_cake"),
            ("棕色地毯", "minecraft:brown_carpet"),
            ("棕色混凝土", "minecraft:brown_concrete"),
            ("棕色混凝土粉末", "minecraft:brown_concrete_powder"),
            ("棕色带釉陶瓦", "minecraft:brown_glazed_terracotta"),
            ("棕色蘑菇", "minecraft:brown_mushroom"),
            ("棕色蘑菇方块", "minecraft:brown_mushroom_block"),
            ("棕色潜影盒", "minecraft:brown_shulker_box"),
            ("棕色染色玻璃", "minecraft:brown_stained_glass"),
            ("棕色染色玻璃板", "minecraft:brown_stained_glass_pane"),
            ("棕色陶瓦", "minecraft:brown_terracotta"),
            ("棕色羊毛", "minecraft:brown_wool"),
            ("气泡柱", "minecraft:bubble_column"),
            ("气泡珊瑚", "minecraft:bubble_coral"),
            ("气泡珊瑚块", "minecraft:bubble_coral_block"),
            ("气泡珊瑚扇", "minecraft:bubble_coral_fan"),
            ("气泡珊瑚扇", "minecraft:bubble_coral_wall_fan"),
            ("紫水晶母岩", "minecraft:budding_amethyst"),
            ("蛋糕", "minecraft:cake"),
            ("方解石", "minecraft:calcite"),
            ("校频幽匿感测体", "minecraft:calibrated_sculk_sensor"),
            ("营火", "minecraft:campfire"),
            ("蜡烛", "minecraft:candle"),
            ("插上蜡烛的蛋糕", "minecraft:candle_cake"),
            ("胡萝卜", "minecraft:carrots"),
            ("制图台", "minecraft:cartography_table"),
            ("雕刻南瓜", "minecraft:carved_pumpkin"),
            ("炼药锅", "minecraft:cauldron"),
            ("洞穴空气", "minecraft:cave_air"),
            ("洞穴藤蔓", "minecraft:cave_vines"),
            ("洞穴藤蔓植株", "minecraft:cave_vines_plant"),
            ("锁链", "minecraft:chain"),
            ("连锁型命令方块", "minecraft:chain_command_block"),
            ("樱花木按钮", "minecraft:cherry_button"),
            ("樱花木门", "minecraft:cherry_door"),
            ("樱花木栅栏", "minecraft:cherry_fence"),
            ("樱花木栅栏门", "minecraft:cherry_fence_gate"),
            ("悬挂式樱花木告示牌", "minecraft:cherry_hanging_sign"),
            ("樱花树叶", "minecraft:cherry_leaves"),
            ("樱花原木", "minecraft:cherry_log"),
            ("樱花木板", "minecraft:cherry_planks"),
            ("樱花木压力板", "minecraft:cherry_pressure_plate"),
            ("樱花树苗", "minecraft:cherry_sapling"),
            ("樱花木告示牌", "minecraft:cherry_sign"),
            ("樱花木台阶", "minecraft:cherry_slab"),
            ("樱花木楼梯", "minecraft:cherry_stairs"),
            ("樱花木活板门", "minecraft:cherry_trapdoor"),
            ("悬挂式樱花木告示牌", "minecraft:cherry_wall_hanging_sign"),
            ("樱花木告示牌", "minecraft:cherry_wall_sign"),
            ("樱花木", "minecraft:cherry_wood"),
            ("箱子", "minecraft:chest"),
            ("开裂的铁砧", "minecraft:chipped_anvil"),
            ("雕纹书架", "minecraft:chiseled_bookshelf"),
            ("雕纹铜块", "minecraft:chiseled_copper"),
            ("雕纹深板岩", "minecraft:chiseled_deepslate"),
            ("雕纹下界砖块", "minecraft:chiseled_nether_bricks"),
            ("雕纹磨制黑石", "minecraft:chiseled_polished_blackstone"),
            ("雕纹石英块", "minecraft:chiseled_quartz_block"),
            ("雕纹红砂岩", "minecraft:chiseled_red_sandstone"),
            ("雕纹砂岩", "minecraft:chiseled_sandstone"),
            ("雕纹石砖", "minecraft:chiseled_stone_bricks"),
            ("雕纹凝灰岩", "minecraft:chiseled_tuff"),
            ("雕纹凝灰岩砖", "minecraft:chiseled_tuff_bricks"),
            ("紫颂花", "minecraft:chorus_flower"),
            ("紫颂植株", "minecraft:chorus_plant"),
            ("黏土", "minecraft:clay"),
            ("煤炭块", "minecraft:coal_block"),
            ("砂土", "minecraft:coarse_dirt"),
            ("深板岩圆石", "minecraft:cobbled_deepslate"),
            ("深板岩圆石台阶", "minecraft:cobbled_deepslate_slab"),
            ("深板岩圆石楼梯", "minecraft:cobbled_deepslate_stairs"),
            ("深板岩圆石墙", "minecraft:cobbled_deepslate_wall"),
            ("圆石", "minecraft:cobblestone"),
            ("圆石台阶", "minecraft:cobblestone_slab"),
            ("圆石楼梯", "minecraft:cobblestone_stairs"),
            ("圆石墙", "minecraft:cobblestone_wall"),
            ("蜘蛛网", "minecraft:cobweb"),
            ("可可果", "minecraft:cocoa"),
            ("命令方块", "minecraft:command_block"),
            ("红石比较器", "minecraft:comparator"),
            ("堆肥桶", "minecraft:composter"),
            ("潮涌核心", "minecraft:conduit"),
            ("铜块", "minecraft:copper_block"),
            ("铜灯", "minecraft:copper_bulb"),
            ("铜门", "minecraft:copper_door"),
            ("铜格栅", "minecraft:copper_grate"),
            ("铜矿石", "minecraft:copper_ore"),
            ("铜活板门", "minecraft:copper_trapdoor"),
            ("矢车菊", "minecraft:cornflower"),
            ("裂纹深板岩砖", "minecraft:cracked_deepslate_bricks"),
            ("裂纹深板岩瓦", "minecraft:cracked_deepslate_tiles"),
            ("裂纹下界砖块", "minecraft:cracked_nether_bricks"),
            ("裂纹磨制黑石砖", "minecraft:cracked_polished_blackstone_bricks"),
            ("裂纹石砖", "minecraft:cracked_stone_bricks"),
            ("合成器", "minecraft:crafter"),
            ("工作台", "minecraft:crafting_table"),
            ("苦力怕的头", "minecraft:creeper_head"),
            ("苦力怕的头", "minecraft:creeper_wall_head"),
            ("绯红木按钮", "minecraft:crimson_button"),
            ("绯红木门", "minecraft:crimson_door"),
            ("绯红木栅栏", "minecraft:crimson_fence"),
            ("绯红木栅栏门", "minecraft:crimson_fence_gate"),
            ("绯红菌", "minecraft:crimson_fungus"),
            ("悬挂式绯红木告示牌", "minecraft:crimson_hanging_sign"),
            ("绯红菌核", "minecraft:crimson_hyphae"),
            ("绯红菌岩", "minecraft:crimson_nylium"),
            ("绯红木板", "minecraft:crimson_planks"),
            ("绯红木压力板", "minecraft:crimson_pressure_plate"),
            ("绯红菌索", "minecraft:crimson_roots"),
            ("绯红木告示牌", "minecraft:crimson_sign"),
            ("绯红木台阶", "minecraft:crimson_slab"),
            ("绯红木楼梯", "minecraft:crimson_stairs"),
            ("绯红菌柄", "minecraft:crimson_stem"),
            ("绯红木活板门", "minecraft:crimson_trapdoor"),
            ("悬挂式绯红木告示牌", "minecraft:crimson_wall_hanging_sign"),
            ("绯红木告示牌", "minecraft:crimson_wall_sign"),
            ("哭泣的黑曜石", "minecraft:crying_obsidian"),
            ("切制铜块", "minecraft:cut_copper"),
            ("切制铜台阶", "minecraft:cut_copper_slab"),
            ("切制铜楼梯", "minecraft:cut_copper_stairs"),
            ("切制红砂岩", "minecraft:cut_red_sandstone"),
            ("切制红砂岩台阶", "minecraft:cut_red_sandstone_slab"),
            ("切制砂岩", "minecraft:cut_sandstone"),
            ("切制砂岩台阶", "minecraft:cut_sandstone_slab"),
            ("青色旗帜", "minecraft:cyan_banner"),
            ("青色床", "minecraft:cyan_bed"),
            ("青色蜡烛", "minecraft:cyan_candle"),
            ("插上青色蜡烛的蛋糕", "minecraft:cyan_candle_cake"),
            ("青色地毯", "minecraft:cyan_carpet"),
            ("青色混凝土", "minecraft:cyan_concrete"),
            ("青色混凝土粉末", "minecraft:cyan_concrete_powder"),
            ("青色带釉陶瓦", "minecraft:cyan_glazed_terracotta"),
            ("青色潜影盒", "minecraft:cyan_shulker_box"),
            ("青色染色玻璃", "minecraft:cyan_stained_glass"),
            ("青色染色玻璃板", "minecraft:cyan_stained_glass_pane"),
            ("青色陶瓦", "minecraft:cyan_terracotta"),
            ("青色羊毛", "minecraft:cyan_wool"),
            ("损坏的铁砧", "minecraft:damaged_anvil"),
            ("蒲公英", "minecraft:dandelion"),
            ("深色橡木按钮", "minecraft:dark_oak_button"),
            ("深色橡木门", "minecraft:dark_oak_door"),
            ("深色橡木栅栏", "minecraft:dark_oak_fence"),
            ("深色橡木栅栏门", "minecraft:dark_oak_fence_gate"),
            ("悬挂式深色橡木告示牌", "minecraft:dark_oak_hanging_sign"),
            ("深色橡树树叶", "minecraft:dark_oak_leaves"),
            ("深色橡木原木", "minecraft:dark_oak_log"),
            ("深色橡木木板", "minecraft:dark_oak_planks"),
            ("深色橡木压力板", "minecraft:dark_oak_pressure_plate"),
            ("深色橡树树苗", "minecraft:dark_oak_sapling"),
            ("深色橡木告示牌", "minecraft:dark_oak_sign"),
            ("深色橡木台阶", "minecraft:dark_oak_slab"),
            ("深色橡木楼梯", "minecraft:dark_oak_stairs"),
            ("深色橡木活板门", "minecraft:dark_oak_trapdoor"),
            ("悬挂式深色橡木告示牌", "minecraft:dark_oak_wall_hanging_sign"),
            ("深色橡木告示牌", "minecraft:dark_oak_wall_sign"),
            ("深色橡木", "minecraft:dark_oak_wood"),
            ("暗海晶石", "minecraft:dark_prismarine"),
            ("暗海晶石台阶", "minecraft:dark_prismarine_slab"),
            ("暗海晶石楼梯", "minecraft:dark_prismarine_stairs"),
            ("阳光探测器", "minecraft:daylight_detector"),
            ("失活的脑纹珊瑚", "minecraft:dead_brain_coral"),
            ("失活的脑纹珊瑚块", "minecraft:dead_brain_coral_block"),
            ("失活的脑纹珊瑚扇", "minecraft:dead_brain_coral_fan"),
            ("失活脑纹珊瑚扇", "minecraft:dead_brain_coral_wall_fan"),
            ("失活的气泡珊瑚", "minecraft:dead_bubble_coral"),
            ("失活的气泡珊瑚块", "minecraft:dead_bubble_coral_block"),
            ("失活的气泡珊瑚扇", "minecraft:dead_bubble_coral_fan"),
            ("失活气泡珊瑚扇", "minecraft:dead_bubble_coral_wall_fan"),
            ("枯萎的灌木", "minecraft:dead_bush"),
            ("失活的火珊瑚", "minecraft:dead_fire_coral"),
            ("失活的火珊瑚块", "minecraft:dead_fire_coral_block"),
            ("失活的火珊瑚扇", "minecraft:dead_fire_coral_fan"),
            ("失活火珊瑚扇", "minecraft:dead_fire_coral_wall_fan"),
            ("失活的鹿角珊瑚", "minecraft:dead_horn_coral"),
            ("失活的鹿角珊瑚块", "minecraft:dead_horn_coral_block"),
            ("失活的鹿角珊瑚扇", "minecraft:dead_horn_coral_fan"),
            ("失活鹿角珊瑚扇", "minecraft:dead_horn_coral_wall_fan"),
            ("失活的管珊瑚", "minecraft:dead_tube_coral"),
            ("失活的管珊瑚块", "minecraft:dead_tube_coral_block"),
            ("失活的管珊瑚扇", "minecraft:dead_tube_coral_fan"),
            ("失活管珊瑚扇", "minecraft:dead_tube_coral_wall_fan"),
            ("饰纹陶罐", "minecraft:decorated_pot"),
            ("深板岩", "minecraft:deepslate"),
            ("深板岩砖台阶", "minecraft:deepslate_brick_slab"),
            ("深板岩砖楼梯", "minecraft:deepslate_brick_stairs"),
            ("深板岩砖墙", "minecraft:deepslate_brick_wall"),
            ("深板岩砖", "minecraft:deepslate_bricks"),
            ("深层煤矿石", "minecraft:deepslate_coal_ore"),
            ("深层铜矿石", "minecraft:deepslate_copper_ore"),
            ("深层钻石矿石", "minecraft:deepslate_diamond_ore"),
            ("深层绿宝石矿石", "minecraft:deepslate_emerald_ore"),
            ("深层金矿石", "minecraft:deepslate_gold_ore"),
            ("深层铁矿石", "minecraft:deepslate_iron_ore"),
            ("深层青金石矿石", "minecraft:deepslate_lapis_ore"),
            ("深层红石矿石", "minecraft:deepslate_redstone_ore"),
            ("深板岩瓦台阶", "minecraft:deepslate_tile_slab"),
            ("深板岩瓦楼梯", "minecraft:deepslate_tile_stairs"),
            ("深板岩瓦墙", "minecraft:deepslate_tile_wall"),
            ("深板岩瓦", "minecraft:deepslate_tiles"),
            ("探测铁轨", "minecraft:detector_rail"),
            ("钻石块", "minecraft:diamond_block"),
            ("闪长岩", "minecraft:diorite"),
            ("闪长岩台阶", "minecraft:diorite_slab"),
            ("闪长岩楼梯", "minecraft:diorite_stairs"),
            ("闪长岩墙", "minecraft:diorite_wall"),
            ("土径", "minecraft:dirt_path"),
            ("发射器", "minecraft:dispenser"),
            ("龙蛋", "minecraft:dragon_egg"),
            ("龙首", "minecraft:dragon_head"),
            ("龙首", "minecraft:dragon_wall_head"),
            ("干海带块", "minecraft:dried_kelp_block"),
            ("滴水石块", "minecraft:dripstone_block"),
            ("投掷器", "minecraft:dropper"),
            ("绿宝石块", "minecraft:emerald_block"),
            ("绿宝石矿石", "minecraft:emerald_ore"),
            ("附魔台", "minecraft:enchanting_table"),
            ("末地折跃门", "minecraft:end_gateway"),
            ("末地传送门", "minecraft:end_portal"),
            ("末地传送门框架", "minecraft:end_portal_frame"),
            ("末地烛", "minecraft:end_rod"),
            ("末地石砖台阶", "minecraft:end_stone_brick_slab"),
            ("末地石砖楼梯", "minecraft:end_stone_brick_stairs"),
            ("末地石砖墙", "minecraft:end_stone_brick_wall"),
            ("末地石砖", "minecraft:end_stone_bricks"),
            ("末影箱", "minecraft:ender_chest"),
            ("斑驳的雕纹铜块", "minecraft:exposed_chiseled_copper"),
            ("斑驳的铜块", "minecraft:exposed_copper"),
            ("斑驳的铜灯", "minecraft:exposed_copper_bulb"),
            ("斑驳的铜门", "minecraft:exposed_copper_door"),
            ("斑驳的铜格栅", "minecraft:exposed_copper_grate"),
            ("斑驳的铜活板门", "minecraft:exposed_copper_trapdoor"),
            ("斑驳的切制铜块", "minecraft:exposed_cut_copper"),
            ("斑驳的切制铜台阶", "minecraft:exposed_cut_copper_slab"),
            ("斑驳的切制铜楼梯", "minecraft:exposed_cut_copper_stairs"),
            ("耕地", "minecraft:farmland"),
            ("蕨", "minecraft:fern"),
            ("火", "minecraft:fire"),
            ("火珊瑚", "minecraft:fire_coral"),
            ("火珊瑚块", "minecraft:fire_coral_block"),
            ("火珊瑚扇", "minecraft:fire_coral_fan"),
            ("火珊瑚扇", "minecraft:fire_coral_wall_fan"),
            ("制箭台", "minecraft:fletching_table"),
            ("花盆", "minecraft:flower_pot"),
            ("盛开的杜鹃花丛", "minecraft:flowering_azalea"),
            ("盛开的杜鹃树叶", "minecraft:flowering_azalea_leaves"),
            ("青蛙卵", "minecraft:frogspawn"),
            ("霜冰", "minecraft:frosted_ice"),
            ("熔炉", "minecraft:furnace"),
            ("镶金黑石", "minecraft:gilded_blackstone"),
            ("玻璃板", "minecraft:glass_pane"),
            ("发光地衣", "minecraft:glow_lichen"),
            ("荧石", "minecraft:glowstone"),
            ("金块", "minecraft:gold_block"),
            ("金矿石", "minecraft:gold_ore"),
            ("花岗岩", "minecraft:granite"),
            ("花岗岩台阶", "minecraft:granite_slab"),
            ("花岗岩楼梯", "minecraft:granite_stairs"),
            ("花岗岩墙", "minecraft:granite_wall"),
            ("草", "minecraft:grass"),
            ("沙砾", "minecraft:gravel"),
            ("灰色旗帜", "minecraft:gray_banner"),
            ("灰色床", "minecraft:gray_bed"),
            ("灰色蜡烛", "minecraft:gray_candle"),
            ("插上灰色蜡烛的蛋糕", "minecraft:gray_candle_cake"),
            ("灰色地毯", "minecraft:gray_carpet"),
            ("灰色混凝土", "minecraft:gray_concrete"),
            ("灰色混凝土粉末", "minecraft:gray_concrete_powder"),
            ("灰色带釉陶瓦", "minecraft:gray_glazed_terracotta"),
            ("灰色潜影盒", "minecraft:gray_shulker_box"),
            ("灰色染色玻璃", "minecraft:gray_stained_glass"),
            ("灰色染色玻璃板", "minecraft:gray_stained_glass_pane"),
            ("灰色陶瓦", "minecraft:gray_terracotta"),
            ("灰色羊毛", "minecraft:gray_wool"),
            ("绿色旗帜", "minecraft:green_banner"),
            ("绿色床", "minecraft:green_bed"),
            ("绿色蜡烛", "minecraft:green_candle"),
            ("插上绿色蜡烛的蛋糕", "minecraft:green_candle_cake"),
            ("绿色地毯", "minecraft:green_carpet"),
            ("绿色混凝土", "minecraft:green_concrete"),
            ("绿色混凝土粉末", "minecraft:green_concrete_powder"),
            ("绿色带釉陶瓦", "minecraft:green_glazed_terracotta"),
            ("绿色潜影盒", "minecraft:green_shulker_box"),
            ("绿色染色玻璃", "minecraft:green_stained_glass"),
            ("绿色染色玻璃板", "minecraft:green_stained_glass_pane"),
            ("绿色陶瓦", "minecraft:green_terracotta"),
            ("绿色羊毛", "minecraft:green_wool"),
            ("砂轮", "minecraft:grindstone"),
            ("垂根", "minecraft:hanging_roots"),
            ("干草捆", "minecraft:hay_block"),
            ("沉重核心", "minecraft:heavy_core"),
            ("重质测重压力板", "minecraft:heavy_weighted_pressure_plate"),
            ("蜂蜜块", "minecraft:honey_block"),
            ("蜜脾块", "minecraft:honeycomb_block"),
            ("漏斗", "minecraft:hopper"),
            ("鹿角珊瑚", "minecraft:horn_coral"),
            ("鹿角珊瑚块", "minecraft:horn_coral_block"),
            ("鹿角珊瑚扇", "minecraft:horn_coral_fan"),
            ("鹿角珊瑚扇", "minecraft:horn_coral_wall_fan"),
            ("虫蚀雕纹石砖", "minecraft:infested_chiseled_stone_bricks"),
            ("虫蚀圆石", "minecraft:infested_cobblestone"),
            ("虫蚀裂纹石砖", "minecraft:infested_cracked_stone_bricks"),
            ("虫蚀深板岩", "minecraft:infested_deepslate"),
            ("虫蚀苔石砖", "minecraft:infested_mossy_stone_bricks"),
            ("虫蚀石头", "minecraft:infested_stone"),
            ("虫蚀石砖", "minecraft:infested_stone_bricks"),
            ("铁栏杆", "minecraft:iron_bars"),
            ("铁块", "minecraft:iron_block"),
            ("铁门", "minecraft:iron_door"),
            ("铁活板门", "minecraft:iron_trapdoor"),
            ("南瓜灯", "minecraft:jack_o_lantern"),
            ("拼图方块", "minecraft:jigsaw"),
            ("唱片机", "minecraft:jukebox"),
            ("丛林木按钮", "minecraft:jungle_button"),
            ("丛林木门", "minecraft:jungle_door"),
            ("丛林木栅栏", "minecraft:jungle_fence"),
            ("丛林木栅栏门", "minecraft:jungle_fence_gate"),
            ("悬挂式丛林木告示牌", "minecraft:jungle_hanging_sign"),
            ("丛林树叶", "minecraft:jungle_leaves"),
            ("丛林原木", "minecraft:jungle_log"),
            ("丛林木板", "minecraft:jungle_planks"),
            ("丛林木压力板", "minecraft:jungle_pressure_plate"),
            ("丛林树苗", "minecraft:jungle_sapling"),
            ("丛林木告示牌", "minecraft:jungle_sign"),
            ("丛林木台阶", "minecraft:jungle_slab"),
            ("丛林木楼梯", "minecraft:jungle_stairs"),
            ("丛林木活板门", "minecraft:jungle_trapdoor"),
            ("悬挂式丛林木告示牌", "minecraft:jungle_wall_hanging_sign"),
            ("丛林木告示牌", "minecraft:jungle_wall_sign"),
            ("丛林木", "minecraft:jungle_wood"),
            ("海带", "minecraft:kelp"),
            ("海带植株", "minecraft:kelp_plant"),
            ("梯子", "minecraft:ladder"),
            ("灯笼", "minecraft:lantern"),
            ("青金石块", "minecraft:lapis_block"),
            ("青金石矿石", "minecraft:lapis_ore"),
            ("大型紫晶芽", "minecraft:large_amethyst_bud"),
            ("大型蕨", "minecraft:large_fern"),
            ("熔岩", "minecraft:lava"),
            ("装有熔岩的炼药锅", "minecraft:lava_cauldron"),
            ("讲台", "minecraft:lectern"),
            ("拉杆", "minecraft:lever"),
            ("光源方块", "minecraft:light"),
            ("淡蓝色旗帜", "minecraft:light_blue_banner"),
            ("淡蓝色床", "minecraft:light_blue_bed"),
            ("淡蓝色蜡烛", "minecraft:light_blue_candle"),
            ("插上淡蓝色蜡烛的蛋糕", "minecraft:light_blue_candle_cake"),
            ("淡蓝色地毯", "minecraft:light_blue_carpet"),
            ("淡蓝色混凝土", "minecraft:light_blue_concrete"),
            ("淡蓝色混凝土粉末", "minecraft:light_blue_concrete_powder"),
            ("淡蓝色带釉陶瓦", "minecraft:light_blue_glazed_terracotta"),
            ("淡蓝色潜影盒", "minecraft:light_blue_shulker_box"),
            ("淡蓝色染色玻璃", "minecraft:light_blue_stained_glass"),
            ("淡蓝色染色玻璃板", "minecraft:light_blue_stained_glass_pane"),
            ("淡蓝色陶瓦", "minecraft:light_blue_terracotta"),
            ("淡蓝色羊毛", "minecraft:light_blue_wool"),
            ("淡灰色旗帜", "minecraft:light_gray_banner"),
            ("淡灰色床", "minecraft:light_gray_bed"),
            ("淡灰色蜡烛", "minecraft:light_gray_candle"),
            ("插上淡灰色蜡烛的蛋糕", "minecraft:light_gray_candle_cake"),
            ("淡灰色地毯", "minecraft:light_gray_carpet"),
            ("淡灰色混凝土", "minecraft:light_gray_concrete"),
            ("淡灰色混凝土粉末", "minecraft:light_gray_concrete_powder"),
            ("淡灰色带釉陶瓦", "minecraft:light_gray_glazed_terracotta"),
            ("淡灰色潜影盒", "minecraft:light_gray_shulker_box"),
            ("淡灰色染色玻璃", "minecraft:light_gray_stained_glass"),
            ("淡灰色染色玻璃板", "minecraft:light_gray_stained_glass_pane"),
            ("淡灰色陶瓦", "minecraft:light_gray_terracotta"),
            ("淡灰色羊毛", "minecraft:light_gray_wool"),
            ("轻质测重压力板", "minecraft:light_weighted_pressure_plate"),
            ("避雷针", "minecraft:lightning_rod"),
            ("丁香", "minecraft:lilac"),
            ("铃兰", "minecraft:lily_of_the_valley"),
            ("睡莲", "minecraft:lily_pad"),
            ("黄绿色旗帜", "minecraft:lime_banner"),
            ("黄绿色床", "minecraft:lime_bed"),
            ("黄绿色蜡烛", "minecraft:lime_candle"),
            ("插上黄绿色蜡烛的蛋糕", "minecraft:lime_candle_cake"),
            ("黄绿色地毯", "minecraft:lime_carpet"),
            ("黄绿色混凝土", "minecraft:lime_concrete"),
            ("黄绿色混凝土粉末", "minecraft:lime_concrete_powder"),
            ("黄绿色带釉陶瓦", "minecraft:lime_glazed_terracotta"),
            ("黄绿色潜影盒", "minecraft:lime_shulker_box"),
            ("黄绿色染色玻璃", "minecraft:lime_stained_glass"),
            ("黄绿色染色玻璃板", "minecraft:lime_stained_glass_pane"),
            ("黄绿色陶瓦", "minecraft:lime_terracotta"),
            ("黄绿色羊毛", "minecraft:lime_wool"),
            ("磁石", "minecraft:lodestone"),
            ("织布机", "minecraft:loom"),
            ("品红色旗帜", "minecraft:magenta_banner"),
            ("品红色床", "minecraft:magenta_bed"),
            ("品红色蜡烛", "minecraft:magenta_candle"),
            ("插上品红色蜡烛的蛋糕", "minecraft:magenta_candle_cake"),
            ("品红色地毯", "minecraft:magenta_carpet"),
            ("品红色混凝土", "minecraft:magenta_concrete"),
            ("品红色混凝土粉末", "minecraft:magenta_concrete_powder"),
            ("品红色带釉陶瓦", "minecraft:magenta_glazed_terracotta"),
            ("品红色潜影盒", "minecraft:magenta_shulker_box"),
            ("品红色染色玻璃", "minecraft:magenta_stained_glass"),
            ("品红色染色玻璃板", "minecraft:magenta_stained_glass_pane"),
            ("品红色陶瓦", "minecraft:magenta_terracotta"),
            ("品红色羊毛", "minecraft:magenta_wool"),
            ("岩浆块", "minecraft:magma_block"),
            ("红树木按钮", "minecraft:mangrove_button"),
            ("红树木门", "minecraft:mangrove_door"),
            ("红树木栅栏", "minecraft:mangrove_fence"),
            ("红树木栅栏门", "minecraft:mangrove_fence_gate"),
            ("悬挂式红树木告示牌", "minecraft:mangrove_hanging_sign"),
            ("红树树叶", "minecraft:mangrove_leaves"),
            ("红树原木", "minecraft:mangrove_log"),
            ("红树木板", "minecraft:mangrove_planks"),
            ("红树木压力板", "minecraft:mangrove_pressure_plate"),
            ("红树胎生苗", "minecraft:mangrove_propagule"),
            ("红树根", "minecraft:mangrove_roots"),
            ("红树木告示牌", "minecraft:mangrove_sign"),
            ("红树木台阶", "minecraft:mangrove_slab"),
            ("红树木楼梯", "minecraft:mangrove_stairs"),
            ("红树木活板门", "minecraft:mangrove_trapdoor"),
            ("悬挂式红树木告示牌", "minecraft:mangrove_wall_hanging_sign"),
            ("红树木告示牌", "minecraft:mangrove_wall_sign"),
            ("红树木", "minecraft:mangrove_wood"),
            ("中型紫晶芽", "minecraft:medium_amethyst_bud"),
            ("西瓜", "minecraft:melon"),
            ("西瓜茎", "minecraft:melon_stem"),
            ("苔藓块", "minecraft:moss_block"),
            ("覆地苔藓", "minecraft:moss_carpet"),
            ("苔石", "minecraft:mossy_cobblestone"),
            ("苔石台阶", "minecraft:mossy_cobblestone_slab"),
            ("苔石楼梯", "minecraft:mossy_cobblestone_stairs"),
            ("苔石墙", "minecraft:mossy_cobblestone_wall"),
            ("苔石砖台阶", "minecraft:mossy_stone_brick_slab"),
            ("苔石砖楼梯", "minecraft:mossy_stone_brick_stairs"),
            ("苔石砖墙", "minecraft:mossy_stone_brick_wall"),
            ("苔石砖", "minecraft:mossy_stone_bricks"),
            ("移动的活塞", "minecraft:moving_piston"),
            ("泥巴", "minecraft:mud"),
            ("泥砖台阶", "minecraft:mud_brick_slab"),
            ("泥砖楼梯", "minecraft:mud_brick_stairs"),
            ("泥砖墙", "minecraft:mud_brick_wall"),
            ("泥砖", "minecraft:mud_bricks"),
            ("沾泥的红树根", "minecraft:muddy_mangrove_roots"),
            ("蘑菇柄", "minecraft:mushroom_stem"),
            ("菌丝体", "minecraft:mycelium"),
            ("下界砖栅栏", "minecraft:nether_brick_fence"),
            ("下界砖台阶", "minecraft:nether_brick_slab"),
            ("下界砖楼梯", "minecraft:nether_brick_stairs"),
            ("下界砖墙", "minecraft:nether_brick_wall"),
            ("下界砖块", "minecraft:nether_bricks"),
            ("下界金矿石", "minecraft:nether_gold_ore"),
            ("下界传送门", "minecraft:nether_portal"),
            ("下界石英矿石", "minecraft:nether_quartz_ore"),
            ("下界苗", "minecraft:nether_sprouts"),
            ("下界疣", "minecraft:nether_wart"),
            ("下界疣块", "minecraft:nether_wart_block"),
            ("下界合金块", "minecraft:netherite_block"),
            ("音符盒", "minecraft:note_block"),
            ("橡木按钮", "minecraft:oak_button"),
            ("橡木门", "minecraft:oak_door"),
            ("橡木栅栏", "minecraft:oak_fence"),
            ("橡木栅栏门", "minecraft:oak_fence_gate"),
            ("悬挂式橡木告示牌", "minecraft:oak_hanging_sign"),
            ("橡树树叶", "minecraft:oak_leaves"),
            ("橡木原木", "minecraft:oak_log"),
            ("橡木压力板", "minecraft:oak_pressure_plate"),
            ("橡树树苗", "minecraft:oak_sapling"),
            ("橡木告示牌", "minecraft:oak_sign"),
            ("橡木台阶", "minecraft:oak_slab"),
            ("橡木楼梯", "minecraft:oak_stairs"),
            ("橡木活板门", "minecraft:oak_trapdoor"),
            ("悬挂式橡木告示牌", "minecraft:oak_wall_hanging_sign"),
            ("橡木告示牌", "minecraft:oak_wall_sign"),
            ("橡木", "minecraft:oak_wood"),
            ("侦测器", "minecraft:observer"),
            ("黑曜石", "minecraft:obsidian"),
            ("赭黄蛙明灯", "minecraft:ochre_froglight"),
            ("灾厄旗帜", "minecraft:ominous_banner"),
            ("橙色旗帜", "minecraft:orange_banner"),
            ("橙色床", "minecraft:orange_bed"),
            ("橙色蜡烛", "minecraft:orange_candle"),
            ("插上橙色蜡烛的蛋糕", "minecraft:orange_candle_cake"),
            ("橙色地毯", "minecraft:orange_carpet"),
            ("橙色混凝土", "minecraft:orange_concrete"),
            ("橙色混凝土粉末", "minecraft:orange_concrete_powder"),
            ("橙色带釉陶瓦", "minecraft:orange_glazed_terracotta"),
            ("橙色潜影盒", "minecraft:orange_shulker_box"),
            ("橙色染色玻璃", "minecraft:orange_stained_glass"),
            ("橙色染色玻璃板", "minecraft:orange_stained_glass_pane"),
            ("橙色陶瓦", "minecraft:orange_terracotta"),
            ("橙色郁金香", "minecraft:orange_tulip"),
            ("橙色羊毛", "minecraft:orange_wool"),
            ("滨菊", "minecraft:oxeye_daisy"),
            ("氧化的雕纹铜块", "minecraft:oxidized_chiseled_copper"),
            ("氧化的铜块", "minecraft:oxidized_copper"),
            ("氧化的铜灯", "minecraft:oxidized_copper_bulb"),
            ("氧化的铜门", "minecraft:oxidized_copper_door"),
            ("氧化的铜格栅", "minecraft:oxidized_copper_grate"),
            ("氧化的铜活板门", "minecraft:oxidized_copper_trapdoor"),
            ("氧化的切制铜块", "minecraft:oxidized_cut_copper"),
            ("氧化的切制铜台阶", "minecraft:oxidized_cut_copper_slab"),
            ("氧化的切制铜楼梯", "minecraft:oxidized_cut_copper_stairs"),
            ("浮冰", "minecraft:packed_ice"),
            ("泥坯", "minecraft:packed_mud"),
            ("珠光蛙明灯", "minecraft:pearlescent_froglight"),
            ("牡丹", "minecraft:peony"),
            ("石化橡木台阶", "minecraft:petrified_oak_slab"),
            ("猪灵的头", "minecraft:piglin_head"),
            ("猪灵的头", "minecraft:piglin_wall_head"),
            ("粉红色旗帜", "minecraft:pink_banner"),
            ("粉红色床", "minecraft:pink_bed"),
            ("粉红色蜡烛", "minecraft:pink_candle"),
            ("插上粉红色蜡烛的蛋糕", "minecraft:pink_candle_cake"),
            ("粉红色地毯", "minecraft:pink_carpet"),
            ("粉红色混凝土", "minecraft:pink_concrete"),
            ("粉红色混凝土粉末", "minecraft:pink_concrete_powder"),
            ("粉红色带釉陶瓦", "minecraft:pink_glazed_terracotta"),
            ("粉红色花簇", "minecraft:pink_petals"),
            ("粉红色潜影盒", "minecraft:pink_shulker_box"),
            ("粉红色染色玻璃", "minecraft:pink_stained_glass"),
            ("粉红色染色玻璃板", "minecraft:pink_stained_glass_pane"),
            ("粉红色陶瓦", "minecraft:pink_terracotta"),
            ("粉红色郁金香", "minecraft:pink_tulip"),
            ("粉红色羊毛", "minecraft:pink_wool"),
            ("活塞", "minecraft:piston"),
            ("活塞头", "minecraft:piston_head"),
            ("瓶子草植株", "minecraft:pitcher_crop"),
            ("瓶子草", "minecraft:pitcher_plant"),
            ("玩家的头", "minecraft:player_head"),
            ("%s的头", "minecraft:player_head.named"),
            ("玩家的头", "minecraft:player_wall_head"),
            ("灰化土", "minecraft:podzol"),
            ("滴水石锥", "minecraft:pointed_dripstone"),
            ("磨制安山岩", "minecraft:polished_andesite"),
            ("磨制安山岩台阶", "minecraft:polished_andesite_slab"),
            ("磨制安山岩楼梯", "minecraft:polished_andesite_stairs"),
            ("磨制玄武岩", "minecraft:polished_basalt"),
            ("磨制黑石", "minecraft:polished_blackstone"),
            ("磨制黑石砖台阶", "minecraft:polished_blackstone_brick_slab"),
            ("磨制黑石砖楼梯", "minecraft:polished_blackstone_brick_stairs"),
            ("磨制黑石砖墙", "minecraft:polished_blackstone_brick_wall"),
            ("磨制黑石砖", "minecraft:polished_blackstone_bricks"),
            ("磨制黑石按钮", "minecraft:polished_blackstone_button"),
            ("磨制黑石压力板", "minecraft:polished_blackstone_pressure_plate"),
            ("磨制黑石台阶", "minecraft:polished_blackstone_slab"),
            ("磨制黑石楼梯", "minecraft:polished_blackstone_stairs"),
            ("磨制黑石墙", "minecraft:polished_blackstone_wall"),
            ("磨制深板岩", "minecraft:polished_deepslate"),
            ("磨制深板岩台阶", "minecraft:polished_deepslate_slab"),
            ("磨制深板岩楼梯", "minecraft:polished_deepslate_stairs"),
            ("磨制深板岩墙", "minecraft:polished_deepslate_wall"),
            ("磨制闪长岩", "minecraft:polished_diorite"),
            ("磨制闪长岩台阶", "minecraft:polished_diorite_slab"),
            ("磨制闪长岩楼梯", "minecraft:polished_diorite_stairs"),
            ("磨制花岗岩", "minecraft:polished_granite"),
            ("磨制花岗岩台阶", "minecraft:polished_granite_slab"),
            ("磨制花岗岩楼梯", "minecraft:polished_granite_stairs"),
            ("磨制凝灰岩", "minecraft:polished_tuff"),
            ("磨制凝灰岩台阶", "minecraft:polished_tuff_slab"),
            ("磨制凝灰岩楼梯", "minecraft:polished_tuff_stairs"),
            ("磨制凝灰岩墙", "minecraft:polished_tuff_wall"),
            ("虞美人", "minecraft:poppy"),
            ("马铃薯", "minecraft:potatoes"),
            ("金合欢树苗盆栽", "minecraft:potted_acacia_sapling"),
            ("绒球葱盆栽", "minecraft:potted_allium"),
            ("杜鹃花丛盆栽", "minecraft:potted_azalea_bush"),
            ("蓝花美耳草盆栽", "minecraft:potted_azure_bluet"),
            ("竹子盆栽", "minecraft:potted_bamboo"),
            ("白桦树苗盆栽", "minecraft:potted_birch_sapling"),
            ("兰花盆栽", "minecraft:potted_blue_orchid"),
            ("棕色蘑菇盆栽", "minecraft:potted_brown_mushroom"),
            ("仙人掌盆栽", "minecraft:potted_cactus"),
            ("樱花树苗盆栽", "minecraft:potted_cherry_sapling"),
            ("矢车菊盆栽", "minecraft:potted_cornflower"),
            ("绯红菌盆栽", "minecraft:potted_crimson_fungus"),
            ("绯红菌索盆栽", "minecraft:potted_crimson_roots"),
            ("蒲公英盆栽", "minecraft:potted_dandelion"),
            ("深色橡树树苗盆栽", "minecraft:potted_dark_oak_sapling"),
            ("枯萎的灌木盆栽", "minecraft:potted_dead_bush"),
            ("蕨盆栽", "minecraft:potted_fern"),
            ("盛开的杜鹃花丛盆栽", "minecraft:potted_flowering_azalea_bush"),
            ("丛林树苗盆栽", "minecraft:potted_jungle_sapling"),
            ("铃兰盆栽", "minecraft:potted_lily_of_the_valley"),
            ("红树胎生苗盆栽", "minecraft:potted_mangrove_propagule"),
            ("橡树树苗盆栽", "minecraft:potted_oak_sapling"),
            ("橙色郁金香盆栽", "minecraft:potted_orange_tulip"),
            ("滨菊盆栽", "minecraft:potted_oxeye_daisy"),
            ("粉红色郁金香盆栽", "minecraft:potted_pink_tulip"),
            ("虞美人盆栽", "minecraft:potted_poppy"),
            ("红色蘑菇盆栽", "minecraft:potted_red_mushroom"),
            ("红色郁金香盆栽", "minecraft:potted_red_tulip"),
            ("云杉树苗盆栽", "minecraft:potted_spruce_sapling"),
            ("火把花盆栽", "minecraft:potted_torchflower"),
            ("诡异菌盆栽", "minecraft:potted_warped_fungus"),
            ("诡异菌索盆栽", "minecraft:potted_warped_roots"),
            ("白色郁金香盆栽", "minecraft:potted_white_tulip"),
            ("凋灵玫瑰盆栽", "minecraft:potted_wither_rose"),
            ("细雪", "minecraft:powder_snow"),
            ("装有细雪的炼药锅", "minecraft:powder_snow_cauldron"),
            ("动力铁轨", "minecraft:powered_rail"),
            ("海晶石", "minecraft:prismarine"),
            ("海晶石砖台阶", "minecraft:prismarine_brick_slab"),
            ("海晶石砖楼梯", "minecraft:prismarine_brick_stairs"),
            ("海晶石砖", "minecraft:prismarine_bricks"),
            ("海晶石台阶", "minecraft:prismarine_slab"),
            ("海晶石楼梯", "minecraft:prismarine_stairs"),
            ("海晶石墙", "minecraft:prismarine_wall"),
            ("南瓜", "minecraft:pumpkin"),
            ("南瓜茎", "minecraft:pumpkin_stem"),
            ("紫色旗帜", "minecraft:purple_banner"),
            ("紫色床", "minecraft:purple_bed"),
            ("紫色蜡烛", "minecraft:purple_candle"),
            ("插上紫色蜡烛的蛋糕", "minecraft:purple_candle_cake"),
            ("紫色地毯", "minecraft:purple_carpet"),
            ("紫色混凝土", "minecraft:purple_concrete"),
            ("紫色混凝土粉末", "minecraft:purple_concrete_powder"),
            ("紫色带釉陶瓦", "minecraft:purple_glazed_terracotta"),
            ("紫色潜影盒", "minecraft:purple_shulker_box"),
            ("紫色染色玻璃", "minecraft:purple_stained_glass"),
            ("紫色染色玻璃板", "minecraft:purple_stained_glass_pane"),
            ("紫色陶瓦", "minecraft:purple_terracotta"),
            ("紫色羊毛", "minecraft:purple_wool"),
            ("紫珀块", "minecraft:purpur_block"),
            ("紫珀柱", "minecraft:purpur_pillar"),
            ("紫珀台阶", "minecraft:purpur_slab"),
            ("紫珀楼梯", "minecraft:purpur_stairs"),
            ("石英块", "minecraft:quartz_block"),
            ("石英砖", "minecraft:quartz_bricks"),
            ("石英柱", "minecraft:quartz_pillar"),
            ("石英台阶", "minecraft:quartz_slab"),
            ("石英楼梯", "minecraft:quartz_stairs"),
            ("铁轨", "minecraft:rail"),
            ("粗铜块", "minecraft:raw_copper_block"),
            ("粗金块", "minecraft:raw_gold_block"),
            ("粗铁块", "minecraft:raw_iron_block"),
            ("红色旗帜", "minecraft:red_banner"),
            ("红色床", "minecraft:red_bed"),
            ("红色蜡烛", "minecraft:red_candle"),
            ("插上红色蜡烛的蛋糕", "minecraft:red_candle_cake"),
            ("红色地毯", "minecraft:red_carpet"),
            ("红色混凝土", "minecraft:red_concrete"),
            ("红色混凝土粉末", "minecraft:red_concrete_powder"),
            ("红色带釉陶瓦", "minecraft:red_glazed_terracotta"),
            ("红色蘑菇", "minecraft:red_mushroom"),
            ("红色蘑菇方块", "minecraft:red_mushroom_block"),
            ("红色下界砖台阶", "minecraft:red_nether_brick_slab"),
            ("红色下界砖楼梯", "minecraft:red_nether_brick_stairs"),
            ("红色下界砖墙", "minecraft:red_nether_brick_wall"),
            ("红色下界砖块", "minecraft:red_nether_bricks"),
            ("红砂岩", "minecraft:red_sandstone"),
            ("红砂岩台阶", "minecraft:red_sandstone_slab"),
            ("红砂岩楼梯", "minecraft:red_sandstone_stairs"),
            ("红砂岩墙", "minecraft:red_sandstone_wall"),
            ("红色潜影盒", "minecraft:red_shulker_box"),
            ("红色染色玻璃", "minecraft:red_stained_glass"),
            ("红色染色玻璃板", "minecraft:red_stained_glass_pane"),
            ("红色陶瓦", "minecraft:red_terracotta"),
            ("红色郁金香", "minecraft:red_tulip"),
            ("红色羊毛", "minecraft:red_wool"),
            ("红石块", "minecraft:redstone_block"),
            ("红石灯", "minecraft:redstone_lamp"),
            ("红石矿石", "minecraft:redstone_ore"),
            ("红石火把", "minecraft:redstone_torch"),
            ("红石火把", "minecraft:redstone_wall_torch"),
            ("红石线", "minecraft:redstone_wire"),
            ("强化深板岩", "minecraft:reinforced_deepslate"),
            ("红石中继器", "minecraft:repeater"),
            ("循环型命令方块", "minecraft:repeating_command_block"),
            ("重生锚", "minecraft:respawn_anchor"),
            ("缠根泥土", "minecraft:rooted_dirt"),
            ("玫瑰丛", "minecraft:rose_bush"),
            ("砂岩", "minecraft:sandstone"),
            ("砂岩台阶", "minecraft:sandstone_slab"),
            ("砂岩楼梯", "minecraft:sandstone_stairs"),
            ("砂岩墙", "minecraft:sandstone_wall"),
            ("脚手架", "minecraft:scaffolding"),
            ("幽匿块", "minecraft:sculk"),
            ("幽匿催发体", "minecraft:sculk_catalyst"),
            ("幽匿感测体", "minecraft:sculk_sensor"),
            ("幽匿尖啸体", "minecraft:sculk_shrieker"),
            ("幽匿脉络", "minecraft:sculk_vein"),
            ("海晶灯", "minecraft:sea_lantern"),
            ("海泡菜", "minecraft:sea_pickle"),
            ("海草", "minecraft:seagrass"),
            ("已设置重生点", "minecraft:set_spawn"),
            ("矮草丛", "minecraft:short_grass"),
            ("菌光体", "minecraft:shroomlight"),
            ("潜影盒", "minecraft:shulker_box"),
            ("骷髅头颅", "minecraft:skeleton_skull"),
            ("骷髅头颅", "minecraft:skeleton_wall_skull"),
            ("小型紫晶芽", "minecraft:small_amethyst_bud"),
            ("小型垂滴叶", "minecraft:small_dripleaf"),
            ("锻造台", "minecraft:smithing_table"),
            ("烟熏炉", "minecraft:smoker"),
            ("平滑玄武岩", "minecraft:smooth_basalt"),
            ("平滑石英块", "minecraft:smooth_quartz"),
            ("平滑石英台阶", "minecraft:smooth_quartz_slab"),
            ("平滑石英楼梯", "minecraft:smooth_quartz_stairs"),
            ("平滑红砂岩", "minecraft:smooth_red_sandstone"),
            ("平滑红砂岩台阶", "minecraft:smooth_red_sandstone_slab"),
            ("平滑红砂岩楼梯", "minecraft:smooth_red_sandstone_stairs"),
            ("平滑砂岩", "minecraft:smooth_sandstone"),
            ("平滑砂岩台阶", "minecraft:smooth_sandstone_slab"),
            ("平滑砂岩楼梯", "minecraft:smooth_sandstone_stairs"),
            ("平滑石头", "minecraft:smooth_stone"),
            ("平滑石头台阶", "minecraft:smooth_stone_slab"),
            ("嗅探兽蛋", "minecraft:sniffer_egg"),
            ("雪", "minecraft:snow"),
            ("灵魂营火", "minecraft:soul_campfire"),
            ("灵魂火", "minecraft:soul_fire"),
            ("灵魂灯笼", "minecraft:soul_lantern"),
            ("灵魂沙", "minecraft:soul_sand"),
            ("灵魂土", "minecraft:soul_soil"),
            ("灵魂火把", "minecraft:soul_torch"),
            ("灵魂火把", "minecraft:soul_wall_torch"),
            ("你的床或已充能的重生锚不存在或已被阻挡", "minecraft:spawn.not_valid"),
            ("刷怪笼", "minecraft:spawner"),
            ("用刷怪蛋交互时：", "minecraft:spawner.desc1"),
            ("设置生物种类", "minecraft:spawner.desc2"),
            ("海绵", "minecraft:sponge"),
            ("孢子花", "minecraft:spore_blossom"),
            ("云杉木按钮", "minecraft:spruce_button"),
            ("云杉木门", "minecraft:spruce_door"),
            ("云杉木栅栏", "minecraft:spruce_fence"),
            ("云杉木栅栏门", "minecraft:spruce_fence_gate"),
            ("悬挂式云杉木告示牌", "minecraft:spruce_hanging_sign"),
            ("云杉树叶", "minecraft:spruce_leaves"),
            ("云杉原木", "minecraft:spruce_log"),
            ("云杉木板", "minecraft:spruce_planks"),
            ("云杉木压力板", "minecraft:spruce_pressure_plate"),
            ("云杉树苗", "minecraft:spruce_sapling"),
            ("云杉木告示牌", "minecraft:spruce_sign"),
            ("云杉木台阶", "minecraft:spruce_slab"),
            ("云杉木楼梯", "minecraft:spruce_stairs"),
            ("云杉木活板门", "minecraft:spruce_trapdoor"),
            ("悬挂式云杉木告示牌", "minecraft:spruce_wall_hanging_sign"),
            ("云杉木告示牌", "minecraft:spruce_wall_sign"),
            ("云杉木", "minecraft:spruce_wood"),
            ("黏性活塞", "minecraft:sticky_piston"),
            ("石砖台阶", "minecraft:stone_brick_slab"),
            ("石砖楼梯", "minecraft:stone_brick_stairs"),
            ("石砖墙", "minecraft:stone_brick_wall"),
            ("石砖", "minecraft:stone_bricks"),
            ("石头按钮", "minecraft:stone_button"),
            ("石头压力板", "minecraft:stone_pressure_plate"),
            ("石头台阶", "minecraft:stone_slab"),
            ("石头楼梯", "minecraft:stone_stairs"),
            ("切石机", "minecraft:stonecutter"),
            ("去皮金合欢原木", "minecraft:stripped_acacia_log"),
            ("去皮金合欢木", "minecraft:stripped_acacia_wood"),
            ("去皮竹块", "minecraft:stripped_bamboo_block"),
            ("去皮白桦原木", "minecraft:stripped_birch_log"),
            ("去皮白桦木", "minecraft:stripped_birch_wood"),
            ("去皮樱花原木", "minecraft:stripped_cherry_log"),
            ("去皮樱花木", "minecraft:stripped_cherry_wood"),
            ("去皮绯红菌核", "minecraft:stripped_crimson_hyphae"),
            ("去皮绯红菌柄", "minecraft:stripped_crimson_stem"),
            ("去皮深色橡木原木", "minecraft:stripped_dark_oak_log"),
            ("去皮深色橡木", "minecraft:stripped_dark_oak_wood"),
            ("去皮丛林原木", "minecraft:stripped_jungle_log"),
            ("去皮丛林木", "minecraft:stripped_jungle_wood"),
            ("去皮红树原木", "minecraft:stripped_mangrove_log"),
            ("去皮红树木", "minecraft:stripped_mangrove_wood"),
            ("去皮橡木原木", "minecraft:stripped_oak_log"),
            ("去皮橡木", "minecraft:stripped_oak_wood"),
            ("去皮云杉原木", "minecraft:stripped_spruce_log"),
            ("去皮云杉木", "minecraft:stripped_spruce_wood"),
            ("去皮诡异菌核", "minecraft:stripped_warped_hyphae"),
            ("去皮诡异菌柄", "minecraft:stripped_warped_stem"),
            ("结构方块", "minecraft:structure_block"),
            ("结构空位", "minecraft:structure_void"),
            ("甘蔗", "minecraft:sugar_cane"),
            ("向日葵", "minecraft:sunflower"),
            ("可疑的沙砾", "minecraft:suspicious_gravel"),
            ("可疑的沙子", "minecraft:suspicious_sand"),
            ("甜浆果丛", "minecraft:sweet_berry_bush"),
            ("高草丛", "minecraft:tall_grass"),
            ("高海草", "minecraft:tall_seagrass"),
            ("标靶", "minecraft:target"),
            ("陶瓦", "minecraft:terracotta"),
            ("遮光玻璃", "minecraft:tinted_glass"),
            ("TNT", "minecraft:tnt"),
            ("火把", "minecraft:torch"),
            ("火把花", "minecraft:torchflower"),
            ("火把花植株", "minecraft:torchflower_crop"),
            ("陷阱箱", "minecraft:trapped_chest"),
            ("试炼刷怪笼", "minecraft:trial_spawner"),
            ("绊线", "minecraft:tripwire"),
            ("绊线钩", "minecraft:tripwire_hook"),
            ("管珊瑚", "minecraft:tube_coral"),
            ("管珊瑚块", "minecraft:tube_coral_block"),
            ("管珊瑚扇", "minecraft:tube_coral_fan"),
            ("管珊瑚扇", "minecraft:tube_coral_wall_fan"),
            ("凝灰岩", "minecraft:tuff"),
            ("凝灰岩砖台阶", "minecraft:tuff_brick_slab"),
            ("凝灰岩砖楼梯", "minecraft:tuff_brick_stairs"),
            ("凝灰岩砖墙", "minecraft:tuff_brick_wall"),
            ("凝灰岩砖", "minecraft:tuff_bricks"),
            ("凝灰岩台阶", "minecraft:tuff_slab"),
            ("凝灰岩楼梯", "minecraft:tuff_stairs"),
            ("凝灰岩墙", "minecraft:tuff_wall"),
            ("海龟蛋", "minecraft:turtle_egg"),
            ("缠怨藤", "minecraft:twisting_vines"),
            ("缠怨藤植株", "minecraft:twisting_vines_plant"),
            ("宝库", "minecraft:vault"),
            ("青翠蛙明灯", "minecraft:verdant_froglight"),
            ("藤蔓", "minecraft:vine"),
            ("虚空空气", "minecraft:void_air"),
            ("火把", "minecraft:wall_torch"),
            ("诡异木按钮", "minecraft:warped_button"),
            ("诡异木门", "minecraft:warped_door"),
            ("诡异木栅栏", "minecraft:warped_fence"),
            ("诡异木栅栏门", "minecraft:warped_fence_gate"),
            ("诡异菌", "minecraft:warped_fungus"),
            ("悬挂式诡异木告示牌", "minecraft:warped_hanging_sign"),
            ("诡异菌核", "minecraft:warped_hyphae"),
            ("诡异菌岩", "minecraft:warped_nylium"),
            ("诡异木板", "minecraft:warped_planks"),
            ("诡异木压力板", "minecraft:warped_pressure_plate"),
            ("诡异菌索", "minecraft:warped_roots"),
            ("诡异木告示牌", "minecraft:warped_sign"),
            ("诡异木台阶", "minecraft:warped_slab"),
            ("诡异木楼梯", "minecraft:warped_stairs"),
            ("诡异菌柄", "minecraft:warped_stem"),
            ("诡异木活板门", "minecraft:warped_trapdoor"),
            ("悬挂式诡异木告示牌", "minecraft:warped_wall_hanging_sign"),
            ("诡异木告示牌", "minecraft:warped_wall_sign"),
            ("诡异疣块", "minecraft:warped_wart_block"),
            ("装有水的炼药锅", "minecraft:water_cauldron"),
            ("涂蜡的雕纹铜块", "minecraft:waxed_chiseled_copper"),
            ("涂蜡的铜块", "minecraft:waxed_copper_block"),
            ("涂蜡的铜灯", "minecraft:waxed_copper_bulb"),
            ("涂蜡的铜门", "minecraft:waxed_copper_door"),
            ("涂蜡的铜格栅", "minecraft:waxed_copper_grate"),
            ("涂蜡的铜活板门", "minecraft:waxed_copper_trapdoor"),
            ("涂蜡的切制铜块", "minecraft:waxed_cut_copper"),
            ("涂蜡的切制铜台阶", "minecraft:waxed_cut_copper_slab"),
            ("涂蜡的切制铜楼梯", "minecraft:waxed_cut_copper_stairs"),
            ("涂蜡的斑驳雕纹铜块", "minecraft:waxed_exposed_chiseled_copper"),
            ("涂蜡的斑驳铜块", "minecraft:waxed_exposed_copper"),
            ("涂蜡的斑驳铜灯", "minecraft:waxed_exposed_copper_bulb"),
            ("涂蜡的斑驳铜门", "minecraft:waxed_exposed_copper_door"),
            ("涂蜡的斑驳铜格栅", "minecraft:waxed_exposed_copper_grate"),
            ("涂蜡的斑驳铜活板门", "minecraft:waxed_exposed_copper_trapdoor"),
            ("涂蜡的斑驳切制铜块", "minecraft:waxed_exposed_cut_copper"),
            ("涂蜡的斑驳切制铜台阶", "minecraft:waxed_exposed_cut_copper_slab"),
            ("涂蜡的斑驳切制铜楼梯", "minecraft:waxed_exposed_cut_copper_stairs"),
            ("涂蜡的氧化雕纹铜块", "minecraft:waxed_oxidized_chiseled_copper"),
            ("涂蜡的氧化铜块", "minecraft:waxed_oxidized_copper"),
            ("涂蜡的氧化铜灯", "minecraft:waxed_oxidized_copper_bulb"),
            ("涂蜡的氧化铜门", "minecraft:waxed_oxidized_copper_door"),
            ("涂蜡的氧化铜格栅", "minecraft:waxed_oxidized_copper_grate"),
            ("涂蜡的氧化铜活板门", "minecraft:waxed_oxidized_copper_trapdoor"),
            ("涂蜡的氧化切制铜块", "minecraft:waxed_oxidized_cut_copper"),
            ("涂蜡的氧化切制铜台阶", "minecraft:waxed_oxidized_cut_copper_slab"),
            ("涂蜡的氧化切制铜楼梯", "minecraft:waxed_oxidized_cut_copper_stairs"),
            ("涂蜡的锈蚀雕纹铜块", "minecraft:waxed_weathered_chiseled_copper"),
            ("涂蜡的锈蚀铜块", "minecraft:waxed_weathered_copper"),
            ("涂蜡的锈蚀铜灯", "minecraft:waxed_weathered_copper_bulb"),
            ("涂蜡的锈蚀铜门", "minecraft:waxed_weathered_copper_door"),
            ("涂蜡的锈蚀铜格栅", "minecraft:waxed_weathered_copper_grate"),
            ("涂蜡的锈蚀铜活板门", "minecraft:waxed_weathered_copper_trapdoor"),
            ("涂蜡的锈蚀切制铜块", "minecraft:waxed_weathered_cut_copper"),
            ("涂蜡的锈蚀切制铜台阶", "minecraft:waxed_weathered_cut_copper_slab"),
            ("涂蜡的锈蚀切制铜楼梯", "minecraft:waxed_weathered_cut_copper_stairs"),
            ("锈蚀的雕纹铜块", "minecraft:weathered_chiseled_copper"),
            ("锈蚀的铜块", "minecraft:weathered_copper"),
            ("锈蚀的铜灯", "minecraft:weathered_copper_bulb"),
            ("锈蚀的铜门", "minecraft:weathered_copper_door"),
            ("锈蚀的铜格栅", "minecraft:weathered_copper_grate"),
            ("锈蚀的铜活板门", "minecraft:weathered_copper_trapdoor"),
            ("锈蚀的切制铜块", "minecraft:weathered_cut_copper"),
            ("锈蚀的切制铜台阶", "minecraft:weathered_cut_copper_slab"),
            ("锈蚀的切制铜楼梯", "minecraft:weathered_cut_copper_stairs"),
            ("垂泪藤", "minecraft:weeping_vines"),
            ("垂泪藤植株", "minecraft:weeping_vines_plant"),
            ("湿海绵", "minecraft:wet_sponge"),
            ("小麦植株", "minecraft:wheat"),
            ("白色旗帜", "minecraft:white_banner"),
            ("白色床", "minecraft:white_bed"),
            ("白色蜡烛", "minecraft:white_candle"),
            ("插上白色蜡烛的蛋糕", "minecraft:white_candle_cake"),
            ("白色地毯", "minecraft:white_carpet"),
            ("白色混凝土", "minecraft:white_concrete"),
            ("白色混凝土粉末", "minecraft:white_concrete_powder"),
            ("白色带釉陶瓦", "minecraft:white_glazed_terracotta"),
            ("白色潜影盒", "minecraft:white_shulker_box"),
            ("白色染色玻璃", "minecraft:white_stained_glass"),
            ("白色染色玻璃板", "minecraft:white_stained_glass_pane"),
            ("白色陶瓦", "minecraft:white_terracotta"),
            ("白色郁金香", "minecraft:white_tulip"),
            ("白色羊毛", "minecraft:white_wool"),
            ("凋灵玫瑰", "minecraft:wither_rose"),
            ("凋灵骷髅头颅", "minecraft:wither_skeleton_skull"),
            ("凋灵骷髅头颅", "minecraft:wither_skeleton_wall_skull"),
            ("黄色旗帜", "minecraft:yellow_banner"),
            ("黄色床", "minecraft:yellow_bed"),
            ("黄色蜡烛", "minecraft:yellow_candle"),
            ("插上黄色蜡烛的蛋糕", "minecraft:yellow_candle_cake"),
            ("黄色地毯", "minecraft:yellow_carpet"),
            ("黄色混凝土", "minecraft:yellow_concrete"),
            ("黄色混凝土粉末", "minecraft:yellow_concrete_powder"),
            ("黄色带釉陶瓦", "minecraft:yellow_glazed_terracotta"),
            ("黄色潜影盒", "minecraft:yellow_shulker_box"),
            ("黄色染色玻璃", "minecraft:yellow_stained_glass"),
            ("黄色染色玻璃板", "minecraft:yellow_stained_glass_pane"),
            ("黄色陶瓦", "minecraft:yellow_terracotta"),
            ("黄色羊毛", "minecraft:yellow_wool"),
            ("僵尸的头", "minecraft:zombie_head"),
            ("僵尸的头", "minecraft:zombie_wall_head"),
        ]
        
        # 添加彩色方块
        colors = [
            ("白色", "white"),
            ("橙色", "orange"),
            ("品红色", "magenta"),
            ("淡蓝色", "light_blue"),
            ("黄色", "yellow"),
            ("黄绿色", "lime"),
            ("粉色", "pink"),
            ("灰色", "gray"),
            ("淡灰色", "light_gray"),
            ("青色", "cyan"),
            ("紫色", "purple"),
            ("蓝色", "blue"),
            ("棕色", "brown"),
            ("绿色", "green"),
            ("红色", "red"),
            ("黑色", "black"),
        ]
        
        # 添加彩色羊毛
        for color_name, color_id in colors:
            self.blocks.append((f"{color_name}羊毛", f"minecraft:{color_id}_wool"))
        
        # 添加彩色玻璃
        for color_name, color_id in colors:
            self.blocks.append((f"{color_name}玻璃", f"minecraft:{color_id}_stained_glass"))
        
        # 添加遮光玻璃（只有一种）
        self.blocks.append(("遮光玻璃", "minecraft:tinted_glass"))
        
        # 添加彩色混凝土
        for color_name, color_id in colors:
            self.blocks.append((f"{color_name}混凝土", f"minecraft:{color_id}_concrete"))
        
        # 添加白色地毯
        self.blocks.append(("地毯", "minecraft:white_carpet"))  # 添加白色地毯
        
        # 生物群系列表
        self.biomes = [
            ("平原", "minecraft:plains"),
            ("沙漠", "minecraft:desert"),
            ("山脉", "minecraft:mountains"),
            ("森林", "minecraft:forest"),
            ("沼泽", "minecraft:swamp"),
            ("丛林", "minecraft:jungle"),
            ("海洋", "minecraft:ocean"),
            ("下界", "minecraft:nether"),
            ("末地", "minecraft:the_end"),
            ("积雪平原", "minecraft:snowy_plains"),
        ]
        
        # 地物生成选项（1.16后失效，仅示例）
        self.structure_options = [
            ("无地物", ""),
            ("生成村庄", "village"),
            ("生成地牢", "dungeon"),
            ("生成要塞", "stronghold"),
            ("生成矿井", "minehaft"),
        ]
        
        # 预设配置
        self.presets = [
            ("经典生存", "classic"),
            ("钻石大陆", "diamond"),
            ("海洋世界", "ocean"),
            ("基岩世界", "bedrock"),
            ("下界生存", "nether"),
            ("末地生存", "end"),
        ]
        
        # 添加排序状态变量
        self.is_sorted = False
        
        self.create_widgets()
        self.layers = []
        self.selected_index = None
        self.checkbox_vars = []
        self.update_layer_display()
    
    def create_widgets(self):
        # 主框架 - 使用网格布局替代pack布局，便于精确控制
        main_frame = ttk.Frame(self.root, padding="10")
        main_frame.pack(fill=tk.BOTH, expand=True)
        
        # 左侧面板 - 方块选择和预设
        left_frame = ttk.LabelFrame(main_frame, text="方块设置", padding="10")
        left_frame.grid(row=0, column=0, rowspan=2, sticky="nsew", padx=(0, 10), pady=(0, 10))
        
        # 方块搜索功能
        search_frame = ttk.Frame(left_frame)
        search_frame.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Label(search_frame, text="搜索方块:").pack(side=tk.LEFT, padx=(0, 5))
        self.search_var = tk.StringVar()
        self.search_entry = ttk.Entry(search_frame, textvariable=self.search_var, width=15)
        self.search_entry.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.search_entry.bind("<KeyRelease>", self.filter_blocks)
        
        # 排序按钮
        sort_btn = ttk.Button(search_frame, text="排序方块", command=self.sort_blocks)
        sort_btn.pack(side=tk.RIGHT, padx=(5, 0))
        
        # 方块选择
        ttk.Label(left_frame, text="可用方块:").pack(anchor=tk.W, pady=(0, 5))
        
        # 创建方块列表框
        self.blocks_frame = ttk.Frame(left_frame)
        self.blocks_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        # 滚动条
        self.blocks_scrollbar = ttk.Scrollbar(self.blocks_frame)
        self.blocks_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        # 方块列表
        self.blocks_listbox = tk.Listbox(self.blocks_frame, height=10, yscrollcommand=self.blocks_scrollbar.set)
        self.blocks_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.blocks_scrollbar.config(command=self.blocks_listbox.yview)
        
        # 填充方块列表
        for block_name, _ in self.blocks:
            self.blocks_listbox.insert(tk.END, block_name)
        
        # 绑定双击事件
        self.blocks_listbox.bind("<Double-1>", self.on_block_double_click)
        
        # 层数设置
        ttk.Label(left_frame, text="层数:").pack(anchor=tk.W, pady=(0, 5))
        self.layer_var = tk.StringVar(value="1")
        self.layer_spinbox = ttk.Spinbox(left_frame, from_=1, to=256, textvariable=self.layer_var, width=10)
        self.layer_spinbox.pack(fill=tk.X, pady=(0, 10))
        
        # 添加方块按钮
        ttk.Button(left_frame, text="添加选中方块", command=self.add_selected_block).pack(fill=tk.X, pady=(0, 10))
        
        # 预设按钮 - 纵向排列
        presets_frame = ttk.LabelFrame(left_frame, text="常用预设", padding="5")
        presets_frame.pack(fill=tk.X, pady=(0, 5))
        
        # 预设按钮容器
        self.presets_container = ttk.Frame(presets_frame)
        self.presets_container.pack(fill=tk.X, expand=True)
        
        # 创建纵向排列的预设按钮
        for i, (text, cmd) in enumerate(self.presets):
            ttk.Button(
                self.presets_container, 
                text=text, 
                command=lambda c=cmd: self.load_preset(c),
                width=15
            ).pack(fill=tk.X, pady=2)
        
        # 关于开发者按钮
        about_btn = ttk.Button(left_frame, text="关于开发者", command=self.show_about)
        about_btn.pack(fill=tk.X, pady=(10, 0))
        
        # 右侧面板 - 世界设置
        right_frame = ttk.LabelFrame(main_frame, text="超平坦世界设置", padding="10")
        right_frame.grid(row=0, column=1, sticky="nsew", pady=(0, 10))
        
        # 已添加的方块层显示
        ttk.Label(right_frame, text="已添加的方块层 (从上到下):").pack(anchor=tk.W, pady=(0, 5))
        layer_frame = ttk.Frame(right_frame)
        layer_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        # 滚动条
        scrollbar = ttk.Scrollbar(layer_frame)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        # 层显示画布
        self.layer_canvas = tk.Canvas(layer_frame, yscrollcommand=scrollbar.set, width=350)
        self.layer_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.config(command=self.layer_canvas.yview)
        
        # 层框架
        self.layers_frame = ttk.Frame(self.layer_canvas)
        self.layer_canvas.create_window((0, 0), window=self.layers_frame, anchor="nw")
        self.layers_frame.bind("<Configure>", lambda e: self.layer_canvas.configure(scrollregion=self.layer_canvas.bbox("all")))
        
        # 层操作按钮
        btn_frame = ttk.Frame(right_frame)
        btn_frame.pack(fill=tk.X, pady=(0, 10))
        ttk.Button(btn_frame, text="上移", command=self.move_up).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="下移", command=self.move_down).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="删除", command=self.remove_layer).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="清空", command=self.clear_layers).pack(side=tk.LEFT, padx=2)
        
        # 世界设置区域
        settings_frame = ttk.LabelFrame(main_frame, text="世界设置", padding="10")
        settings_frame.grid(row=1, column=1, sticky="nsew")
        
        # 生物群系设置
        ttk.Label(settings_frame, text="选择生物群系:").pack(anchor=tk.W, pady=(0, 5))
        self.biome_var = tk.StringVar()
        self.biome_combobox = ttk.Combobox(settings_frame, textvariable=self.biome_var, width=30)
        self.biome_combobox['values'] = [biome[0] for biome in self.biomes]
        self.biome_combobox.pack(fill=tk.X, pady=(0, 10))
        self.biome_combobox.current(0)
        
        # 地物生成选项（1.16后失效）
        ttk.Label(settings_frame, text="地物生成选项 (1.16后失效):").pack(anchor=tk.W, pady=(0, 5))
        self.structure_var = tk.StringVar()
        self.structure_combobox = ttk.Combobox(settings_frame, textvariable=self.structure_var, width=30)
        self.structure_combobox['values'] = [struct[0] for struct in self.structure_options]
        self.structure_combobox.pack(fill=tk.X, pady=(0, 10))
        self.structure_combobox.current(0)
        
        # 生成代码区域
        ttk.Label(settings_frame, text="生成的超平坦代码:").pack(anchor=tk.W, pady=(0, 5))
        self.code_text = tk.Text(settings_frame, height=3, width=50)
        self.code_text.pack(fill=tk.BOTH, expand=True, pady=(0, 5))
        
        # 代码操作按钮
        code_btn_frame = ttk.Frame(settings_frame)
        code_btn_frame.pack(fill=tk.X)
        ttk.Button(code_btn_frame, text="生成超平坦代码", command=self.generate_code).pack(side=tk.LEFT, padx=(0, 5), fill=tk.X, expand=True)
        ttk.Button(code_btn_frame, text="复制代码", command=self.copy_code).pack(side=tk.LEFT, fill=tk.X, expand=True)
        
        # 设置网格权重，使组件能够正确扩展
        main_frame.columnconfigure(0, weight=1)
        main_frame.columnconfigure(1, weight=2)
        main_frame.rowconfigure(0, weight=2)
        main_frame.rowconfigure(1, weight=1)
        
        # 设置默认选中第一个方块
        if len(self.blocks) > 0:
            self.blocks_listbox.selection_set(0)
    
    def filter_blocks(self, event=None):
        search_text = self.search_var.get().lower()
        self.blocks_listbox.delete(0, tk.END)
        
        # 保存当前选中的方块名称
        selected_name = None
        if self.blocks_listbox.curselection():
            selected_idx = self.blocks_listbox.curselection()[0]
            selected_name = self.blocks_listbox.get(selected_idx)
        
        # 临时排序后的方块列表（仅用于显示）
        sorted_blocks = sorted(self.blocks, key=lambda x: x[0]) if self.is_sorted else self.blocks
        
        # 过滤并显示方块
        for block_name, _ in sorted_blocks:
            if search_text in block_name.lower():
                self.blocks_listbox.insert(tk.END, block_name)
        
        # 如果有结果，选中第一个或之前选中的方块
        if self.blocks_listbox.size() > 0:
            if selected_name and selected_name in [self.blocks_listbox.get(i) for i in range(self.blocks_listbox.size())]:
                for i in range(self.blocks_listbox.size()):
                    if self.blocks_listbox.get(i) == selected_name:
                        self.blocks_listbox.selection_set(i)
                        break
            else:
                self.blocks_listbox.selection_set(0)
    
    def sort_blocks(self):
        self.is_sorted = not self.is_sorted
        self.blocks_listbox.delete(0, tk.END)
        
        # 保存当前选中的方块名称
        selected_name = None
        if self.blocks_listbox.curselection():
            selected_idx = self.blocks_listbox.curselection()[0]
            selected_name = self.blocks_listbox.get(selected_idx)
        
        # 排序并显示方块
        sorted_blocks = sorted(self.blocks, key=lambda x: x[0]) if self.is_sorted else self.blocks
        for block_name, _ in sorted_blocks:
            self.blocks_listbox.insert(tk.END, block_name)
        
        # 恢复选中状态
        if selected_name and selected_name in [self.blocks_listbox.get(i) for i in range(self.blocks_listbox.size())]:
            for i in range(self.blocks_listbox.size()):
                if self.blocks_listbox.get(i) == selected_name:
                    self.blocks_listbox.selection_set(i)
                    break
        else:
            if self.blocks_listbox.size() > 0:
                self.blocks_listbox.selection_set(0)
    
    def on_block_double_click(self, event=None):
        self.add_selected_block()
    
    def add_selected_block(self):
        selection = self.blocks_listbox.curselection()
        if not selection:
            messagebox.showwarning("警告", "请先选择一个方块")
            return
        
        block_name = self.blocks_listbox.get(selection[0])
        block_id = next((b[1] for b in self.blocks if b[0] == block_name), None)
        
        if not block_id:
            messagebox.showerror("错误", "未找到方块ID")
            return
        
        try:
            count = int(self.layer_var.get())
            if count <= 0:
                messagebox.showerror("错误", "层数必须大于0")
                return
        except ValueError:
            messagebox.showerror("错误", "请输入有效的层数")
            return
        
        self.layers.append({"name": block_name, "id": block_id, "count": count})
        self.selected_index = None
        self.update_layer_display()
    
    def update_layer_display(self):
        # 清空层显示
        for widget in self.layers_frame.winfo_children():
            widget.destroy()
        self.checkbox_vars = []
        
        # 重新显示所有层
        for i, layer in enumerate(self.layers):
            var = tk.BooleanVar()
            self.checkbox_vars.append(var)
            checkbox = ttk.Checkbutton(
                self.layers_frame, 
                variable=var, 
                command=lambda idx=i: self.select_layer(idx)
            )
            checkbox.grid(row=i, column=0, padx=5, pady=2, sticky="w")
            
            ttk.Label(
                self.layers_frame, 
                text=f"{i+1}. {layer['name']} x{layer['count']}"
            ).grid(row=i, column=1, padx=5, pady=2, sticky="w")
            
            self.layers_frame.rowconfigure(i, weight=1)
        self.layers_frame.columnconfigure(0, weight=0)
        self.layers_frame.columnconfigure(1, weight=1)
    
    def select_layer(self, index):
        for i, var in enumerate(self.checkbox_vars):
            var.set(i == index)
        self.selected_index = index
    
    def move_up(self):
        if self.selected_index is None or self.selected_index <= 0:
            messagebox.showwarning("警告", "请选择可上移的层")
            return
        self.layers[self.selected_index], self.layers[self.selected_index-1] = self.layers[self.selected_index-1], self.layers[self.selected_index]
        self.selected_index -= 1
        self.update_layer_display()
    
    def move_down(self):
        if self.selected_index is None or self.selected_index >= len(self.layers)-1:
            messagebox.showwarning("警告", "请选择可下移的层")
            return
        self.layers[self.selected_index], self.layers[self.selected_index+1] = self.layers[self.selected_index+1], self.layers[self.selected_index]
        self.selected_index += 1
        self.update_layer_display()
    
    def remove_layer(self):
        if self.selected_index is None:
            messagebox.showwarning("警告", "请先选择要删除的层")
            return
        del self.layers[self.selected_index]
        self.selected_index = None
        self.update_layer_display()
    
    def clear_layers(self):
        self.layers = []
        self.selected_index = None
        self.update_layer_display()
    
    def generate_code(self):
        if not self.layers:
            messagebox.showwarning("警告", "请先添加方块层")
            return
        
        # 获取生物群系和地物选项
        biome_name = self.biome_var.get()
        biome_id = next((b[1] for b in self.biomes if b[0] == biome_name), None)
        structure = self.structure_var.get()
        structure_opt = next((s[1] for s in self.structure_options if s[0] == structure), "")
        
        if not biome_id:
            messagebox.showerror("错误", "未找到生物群系ID")
            return
        
        # 生成方块ID列表（从-64层向上排列，需反转顺序）
        code_parts = []
        for layer in reversed(self.layers):
            if layer['count'] > 1:
                code_parts.append(f"{layer['count']}*{layer['id']}")
            else:
                code_parts.append(layer['id'])
        
        # 组合完整代码：方块列表;生物群系;地物选项（1.16后地物选项失效）
        full_code = f"{','.join(code_parts)};{biome_id}"
        if structure_opt:
            full_code += f";{structure_opt}"
        
        self.code_text.delete(1.0, tk.END)
        self.code_text.insert(tk.END, full_code)
    
    def copy_code(self):
        code = self.code_text.get(1.0, tk.END).strip()
        if code:
            self.root.clipboard_clear()
            self.root.clipboard_append(code)
            messagebox.showinfo("成功", "代码已复制到剪贴板")
        else:
            messagebox.showwarning("警告", "没有可复制的代码")
    
    def load_preset(self, preset_type):
        self.clear_layers()
        
        if preset_type == "classic":
            # 经典生存：草方块(1)、泥土(2)、基岩(1)
            self.add_preset_layer("草方块", 1)
            self.add_preset_layer("泥土", 2)
            self.add_preset_layer("基岩", 1)
            self.biome_combobox.set("平原")
        
        elif preset_type == "diamond":
            # 钻石大陆：钻石矿石(5)、石头(10)、基岩(1)
            self.add_preset_layer("钻石矿石", 5)
            self.add_preset_layer("石头", 10)
            self.add_preset_layer("基岩", 1)
            self.biome_combobox.set("山脉")
        
        elif preset_type == "ocean":
            # 海洋世界：水(20)、沙子(2)、基岩(1)
            self.add_preset_layer("水", 20)
            self.add_preset_layer("沙子", 2)
            self.add_preset_layer("基岩", 1)
            self.biome_combobox.set("海洋")
        
        elif preset_type == "bedrock":
            # 基岩世界：基岩(256)
            self.add_preset_layer("基岩", 256)
            self.biome_combobox.set("末地")
        
        elif preset_type == "nether":
            # 下界生存：下界岩(20)、灵魂沙(2)、基岩(1)
            self.add_preset_layer("下界岩", 20)
            self.add_preset_layer("泥土", 2)  # 替代灵魂沙（示例）
            self.add_preset_layer("基岩", 1)
            self.biome_combobox.set("下界")
        
        elif preset_type == "end":
            # 末地生存：末地石(20)、紫珀块(1)、基岩(1)
            self.add_preset_layer("末地石", 20)
            self.add_preset_layer("玻璃", 1)  # 替代紫珀块（示例）
            self.add_preset_layer("基岩", 1)
            self.biome_combobox.set("末地")
        
        # 加载预设后自动生成代码
        self.generate_code()
        self.update_layer_display()
    
    def add_preset_layer(self, block_name, count):
        block_id = next((b[1] for b in self.blocks if b[0] == block_name), None)
        if block_id:
            self.layers.append({"name": block_name, "id": block_id, "count": count})
    
    def show_about(self):
        # 创建关于开发者窗口
        about_window = tk.Toplevel(self.root)
        about_window.title("关于开发者")
        about_window.geometry("400x300")
        about_window.resizable(False, False)
        
        # 开发者信息
        about_frame = ttk.Frame(about_window, padding="20")
        about_frame.pack(fill=tk.BOTH, expand=True)
        
        ttk.Label(about_frame, text="此脚本由", font=("Arial", 12)).grid(row=0, column=0, sticky="w", pady=(0, 5))
        ttk.Label(about_frame, text="我一定天下无敌【网名】", font=("Arial", 12, "bold")).grid(row=0, column=1, sticky="w", pady=(0, 5))
        ttk.Label(about_frame, text="借助AI辅助开发制作。", font=("Arial", 12)).grid(row=1, column=0, columnspan=2, sticky="w", pady=(0, 15))
        
        ttk.Label(about_frame, text="如需联系：", font=("Arial", 12)).grid(row=2, column=0, columnspan=2, sticky="w", pady=(0, 10))
        
        ttk.Label(about_frame, text="QQ:", font=("Arial", 10)).grid(row=3, column=0, sticky="w", pady=2)
        ttk.Label(about_frame, text="548733917", font=("Arial", 10)).grid(row=3, column=1, sticky="w", pady=2)
        
        ttk.Label(about_frame, text="邮箱:", font=("Arial", 10)).grid(row=4, column=0, sticky="w", pady=2)
        ttk.Label(about_frame, text="548733917@qq.com", font=("Arial", 10)).grid(row=4, column=1, sticky="w", pady=2)
        
        ttk.Label(about_frame, text="快手ID:", font=("Arial", 10)).grid(row=5, column=0, sticky="w", pady=2)
        ttk.Label(about_frame, text="ac180000", font=("Arial", 10)).grid(row=5, column=1, sticky="w", pady=2)
        
        ttk.Label(about_frame, text="哔哩哔哩UUID:", font=("Arial", 10)).grid(row=6, column=0, sticky="w", pady=2)
        ttk.Label(about_frame, text="3493078665005855", font=("Arial", 10)).grid(row=6, column=1, sticky="w", pady=2)
        
        # 复制按钮
        copy_btn = ttk.Button(about_frame, text="复制所有ID", command=lambda: self.copy_contact_info(about_window))
        copy_btn.grid(row=7, column=0, columnspan=2, pady=(15, 0))
        
        # 支持开发者按钮
        support_btn = ttk.Button(about_frame, text="支持开发者", command=self.show_support)
        support_btn.grid(row=8, column=0, columnspan=2, pady=(5, 0))
        
        # 设置网格权重
        about_frame.columnconfigure(0, weight=0)
        about_frame.columnconfigure(1, weight=1)
        for i in range(9):
            about_frame.rowconfigure(i, weight=1)
    
    def copy_contact_info(self, window):
        # 复制所有联系方式
        contact_info = "QQ: 548733917\n邮箱: 548733917@qq.com\n快手ID: ac180000\n哔哩哔哩UUID: 3493078665005855"
        self.root.clipboard_clear()
        self.root.clipboard_append(contact_info)
        messagebox.showinfo("成功", "联系方式已复制到剪贴板")
    
    def show_support(self):
        # 创建支持开发者窗口
        support_window = tk.Toplevel(self.root)
        support_window.title("支持开发者")
        support_window.geometry("350x400")
        support_window.resizable(False, False)
        
        # 支持信息
        support_frame = ttk.Frame(support_window, padding="20")
        support_frame.pack(fill=tk.BOTH, expand=True)
        
        ttk.Label(support_frame, text="感谢您的支持！", font=("Arial", 14, "bold")).pack(pady=(0, 15))
        
        # 图片区域 - 预留Base64编码位置
        image_frame = ttk.Frame(support_frame)
        image_frame.pack(pady=(0, 15))
        
        # 在这里添加300x300的图片Base64编码
        image_data = ""
        photo = tk.PhotoImage(data=image_data)
        image_label = ttk.Label(image_frame, image=photo)
        image_label.image = photo  # 保持引用
        image_label.pack()
        
        # 临时使用一个占位标签
        ttk.Label(image_frame, text="(这里将显示支持二维码图片)", font=("Arial", 10)).pack()
        
        ttk.Label(support_frame, text="您的每一份支持都将激励我继续开发更好的工具！", font=("Arial", 10)).pack(pady=(0, 15))
        
        ttk.Button(support_frame, text="确定", command=support_window.destroy).pack()

if __name__ == "__main__":
    if sys.platform == "win32":
        import ctypes
        ctypes.windll.user32.ShowWindow(ctypes.windll.kernel32.GetConsoleWindow(), 0)
    
    root = tk.Tk()
    app = MinecraftFlatWorldGenerator(root)
    root.mainloop()